...
BACKGROUND_IMAGES = {
    "P1": r"C:\Users\chu2\Downloads\1659946255182.jpg",
    "P2": r"C:\Users\chu2\Pictures\Screenshots\Screenshot 2025-07-30 202112.png",
    "P3": r"C:\Users\chu2\Pictures\Screenshots\Screenshot 2025-07-30 202138.png",
    "P4": r"C:\Users\chu2\Pictures\Screenshots\Screenshot 2025-07-30 202158.png",
    "P5": r"C:\Users\chu2\Pictures\Screenshots\Screenshot 2025-07-30 202214.png",
...
class WebcamApp:
    ...
    def update_preview(self):
        ...
        if self.countdown_start_time is not None:
            ...
            if current_countdown > 0:
                neon_green = (57, 255, 20)  # BGR for neon green
                font = cv2.FONT_HERSHEY_SIMPLEX
                font_scale = 8  # much bigger
                font_thickness = 18
                ...
                # Determine text size for centering
                ...
                cv2.putText(
                    frame_copy, text, (x, y),
                    font, font_scale, neon_green, font_thickness, cv2.LINE_AA
                )
    ...
    def show_please_wait_screen(self):
        ...
        # Place both labels in a container frame, centered
        text_frame = tk.Frame(wait_frame, bg='white')
        ...
        self.please_wait_label.pack(anchor='center')
        ...
        self.dots_label.pack(anchor="center", pady=(0,30))  # pad below as needed
    ...
    def process_image(self):
        # Remove background ONCE (using REMBG or OpenCV)
        ...
        for bg_key, bg_path in BACKGROUND_IMAGES.items():
            label = BACKGROUND_LABELS.get(bg_key, bg_key)
            if not os.path.exists(bg_path):
                continue  # skip missing backgrounds

            # Prepare background and composite
            background_img = Image.open(bg_path).convert('RGBA').resize((orig_width, orig_height), Image.Resampling.LANCZOS)
            final_image = Image.alpha_composite(background_img, subject_no_bg).convert('RGB')

            # Save composite to disk
            composite_filename = f"final_composite_{bg_key}_{TIMESTAMP}.png"
            composite_path = os.path.join(self.captures_dir, composite_filename)
            final_image.save(composite_path)

            # Upload to Drive and store shareable link
            photo = drive.CreateFile({'parents': [{'id': folder_id}], 'title': composite_filename})
            photo.SetContentFile(composite_path)
            photo.Upload()
            file_link = 'https://drive.google.com/file/d/' + photo['id'] + '/view?usp=sharing'
            links.append((label, file_link))

        # Generate a simple HTML landing page with all links
        html_path, html_drive_link = self.create_landing_page(links, drive, folder_id)
        # Generate QR code for the HTML landing page
        qr = qrcode.make(html_drive_link)
        qr.save("qr.png")
        # Show first composite as preview (optional)
        preview_img = cv2.imread(os.path.join(self.captures_dir, f"final_composite_{list(BACKGROUND_IMAGES.keys())[0]}_{TIMESTAMP}.png"))
        self.root.after(0, lambda: self.show_final_result(preview_img, qr))

    def create_landing_page(self, links, drive, folder_id):
        html_path = os.path.join(self.captures_dir, f"all_backgrounds_{TIMESTAMP}.html")
        with open(html_path, 'w') as f:
            f.write("<html><head><title>Your Photos</title></head><body>")
            f.write('<h1>Your photos with all backgrounds</h1><ul>')
            for label, link in links:
                f.write(f'<li><a href="{link}" target="_blank">{label}</a></li>')
            f.write('</ul></body></html>')
        # Upload HTML file to Drive
        html_drive = drive.CreateFile({'parents': [{'id': folder_id}], 'title': os.path.basename(html_path), 'mimeType': 'text/html'})
        html_drive.SetContentFile(html_path)
        html_drive.Upload()
        html_drive_link = 'https://drive.google.com/file/d/' + html_drive['id'] + '/view?usp=sharing'
        return html_path, html_drive_link
